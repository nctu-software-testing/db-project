...
class StatController extends BaseController
{
    ...
    public function getBusinessStat(Request $request)
    {
        return view('stat.business')
            ->with('title', '統計資料');
    }
    ...
    public function postBusinessStat(Request $request)
    {
        $type = intval(request('type', -1));
        $uid = session('user.id');
        $filterData = function ($data) {
        ...
        if ($type === 1) {
            $sellProducts = OrderProduct::join('order', 'order.id', 'order_product.order_id')
                ->join('on_product', 'on_product.id', 'order_product.product_id')
                ->join('category', 'category.id', 'on_product.category_id')
            ...
            if ($dlist) {
                ...
                foreach ($locationProducts as $d) {
                    $d->district = $zipCodeMap[strval($d->zip_code)];
                }

                $result = $filterData($locationProducts);
